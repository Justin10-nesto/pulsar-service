...
docker run --rm \
    -v pulsar-service_zookeeper-data:/source:ro \
    -v "$(pwd)/$BACKUP_DIR":/backup \
    alpine tar czf /backup/zookeeper-data.tar.gz -C /source .
echo "✓ ZooKeeper data backed up"

docker run --rm \
    -v pulsar-service_bookkeeper-data:/source:ro \
    -v "$(pwd)/$BACKUP_DIR":/backup \
    alpine tar czf /backup/bookkeeper-data.tar.gz -C /source .
echo "✓ BookKeeper data backed up"

docker run --rm \
    -v pulsar-service_pulsar-data:/source:ro \
    -v "$(pwd)/$BACKUP_DIR":/backup \
    alpine tar czf /backup/pulsar-data.tar.gz -C /source .
echo "✓ Pulsar data backed up"
